...
Das System soll Nutzern ermöglichen, Paddelboote an festgelegten Standorten sowohl über eine Website als auch über mobile Endgeräte buchen und bezahlen zu können. Die Verfügbarkeit sowie die Verleihstationen der Boote sollen im Produkt ersichtlich sein. Außerdem soll bei einer abgeschlossenen Buchung das Boot freischaltet werden. Die mechanische Ausführung vor Ort ist dabei nicht Bestandteil des Projektes.
...
Im Rahmen dieses Projektes soll ein dezentrales, cloud-basiertes Buchungsportal erarbeitet werden, um eine Etablierung der Boote auf dem Markt zu ermöglichen.
